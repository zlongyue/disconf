...
/**
 * 扩展disconf默认提供类，使之支持注解动态加载
 * 
 * 具有 reloadable 的 property bean 扩展了 DefaultPropertyPlaceholderConfigurer 特性如下：
 * 1. 启动时 监控 动态config，并维护它们与相应bean的关系 2. 当动态config变动时，此configurer会进行reload 3.
 * reload 时会 compare config value, and set value for beans
 */
public class ReloadingPropertyPlaceholderConfigurer extends DefaultPropertyPlaceholderConfigurer
		implements InitializingBean, DisposableBean, IReloadablePropertiesListener, ApplicationContextAware {
	...
	private void disconfDynamic(String strVal) {
		...
		while (startIndex != -1) {
			...
			if (endIndex != -1) {
				if (currentBeanName != null && currentPropertyName != null) {
					...
					/**
					 * val 为判断是否在注册中心获取值的判断
					 */
					Object val = null;
					if (null != lastMergedProperties) {
						val = lastMergedProperties.getProperty(placeholder);
					}
					if (val != null) {
						if (dynamic == null) {
	...
	protected Properties mergeProperties() throws IOException {
		...
		/**
		 * 扩展--》 把配置中心的数据存放到environment中
		 */
		ConfigurableEnvironment environment = (ConfigurableEnvironment) applicationContext.getEnvironment();
		if (null != environment && null != properties) {
			environment.getPropertySources().addFirst(new PropertiesPropertySource("disconf", properties));
		}
	...
	*/
	...
	protected void processProperties(ConfigurableListableBeanFactory beanFactoryToProcess, Properties props)
		...
		for (int i = 0; i < beanNames.length; i++) {
			...
			// net.unicon.iamlabs.spring.properties.example.net
	...
	private class PlaceholderResolvingBeanDefinitionVisitor extends BeanDefinitionVisitor {
		...
		protected void visitPropertyValues(MutablePropertyValues pvs) {
			...
			if (null == pvArray || pvArray.length == 0) {
				/**
				 * 通过获取Field字段上的是否有@Value注解来组装Dynamic信息，
				 * 且必须是从配置中心获取字段值的字段属性
				 */
					...
					ReflectionUtils.doWithFields(clazz, new ReflectionUtils.FieldCallback() {
						public void doWith(Field field) throws IllegalArgumentException, IllegalAccessException {
							...
							if (null != value) {
								...
								String spelVal = value.value();
								disconfDynamic(spelVal);
							}
						}
					});
				} catch (Exception e) {
	...
	*/
	...
	public void setProperties(Properties... propertiesArray) {
